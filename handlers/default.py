...
from data.translations import _, ru_texts, uzb_texts
...
from keyboards.payment_confirm_reject_kb import PayConfirmAction, PayConfirmCallback, get_payment_confirm_reject_markup
from keyboards.products_kb import products_user_kb, free_products
from keyboards.select_tariffs_kb import action_for_get_paid_2, action_for_get_paid, action_for_get_paid_3, \
    get_back_kb_button
...
async def tariff_prev_button(
    ...
    if prev_state == AllTariffsState.tariff_name:
        ...
        # Чтобы заново отправить меню с названиями тарифов нам нужно сначало
...
async def paid_confirm_reject(
    ...
    if callback_data.action == PayConfirmAction.CONFIRM:
        ...
        if not user.tariff_id:
            alert_text = _(ru_texts['paid_admin_tariff_success'], user_lang)
